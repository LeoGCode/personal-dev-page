...
Every landing includes a contact form that flows through a strict pipeline: client-side validation with Zod, server-side re-validation in a Next.js route handler, and then a JSON-RPC call to Odoo CRM. Each lead is automatically tagged with the source landing, service type, locale, and variant — so the sales view in Odoo shows exactly where every lead came from.
